...
| 页面类型 | `ListPage` | 映射到 `assets/patterns/design-patterns.json` 的 key |
...
查询 `assets/patterns/design-patterns.json`，匹配最接近的页面类型。
...
└── Content [flex:1, flexDirection:column]
    ...
    └── Main [padding:24px]
        ├── FilterBar (RangePicker + Select，默认实时查询，无查询/重置按钮)
